...
def verify_manifests_and_syntax() -> None:
    ...
    manifest_paths = [
        ...
        ROOT / ".Gemini-plugin/plugin.json",
        ROOT / ".Gemini-plugin/marketplace.json",
...
def verify_hook_install_flow() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="caveman-verify-") as temp_root:
        ...
        ensure((Gemini_dir / ".caveman-active").read_text() == "full", "activation flag should default to full")
        ...
        ensure((Gemini_dir / ".caveman-active").read_text() == "ultra", "CAVEMAN_DEFAULT_MODE=ultra should set flag to ultra")
        ...
        ensure(not (Gemini_dir / ".caveman-active").exists(), "off mode should remove flag file")
        ...
        ensure(not (Gemini_dir / ".caveman-active").exists(), "/caveman with off default should not write flag")

        # Reset back to full for subsequent tests
        (Gemini_dir / ".caveman-active").write_text("full")
        ...
        ensure((Gemini_dir / ".caveman-active").read_text() == "ultra", "mode tracker did not record ultra")
        ...
        ensure(not (Gemini_dir / ".caveman-active").exists(), "normal mode should remove flag file")

        (Gemini_dir / ".caveman-active").write_text("wenyan-ultra")
        statusline = run(
        ...
        settings_after = read_json(Gemini_dir / "settings.json")
        ensure(settings_after == existing_settings, "uninstall.sh did not restore non-caveman settings")
        ensure(not (Gemini_dir / ".caveman-active").exists(), "uninstall.sh should remove flag file")
    ...
    with tempfile.TemporaryDirectory(prefix="caveman-verify-fresh-") as temp_root:
        ...
        run(["bash", "hooks/uninstall.sh"], env={"HOME": str(home)})
        ensure(read_json(Gemini_dir / "settings.json") == {}, "fresh uninstall should leave empty settings")

    print("Gemini hook install/uninstall flow OK")
