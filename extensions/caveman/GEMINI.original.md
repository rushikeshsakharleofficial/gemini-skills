...
Caveman makes AI coding agents respond in compressed, caveman-style prose — cutting ~65-75% of output tokens while keeping full technical accuracy. It ships as a Gemini CLI plugin, a Codex plugin, a Gemini CLI extension, and as agent rule files for Cursor, Windsurf, Cline, Copilot, and 40+ others via `npx skills`.
...
Three hooks ship in `hooks/`. They communicate via a flag file at `~/.Gemini/.caveman-active`.

```
SessionStart hook ──writes "full"──▶ ~/.Gemini/.caveman-active ◀──writes mode── UserPromptSubmit hook
                                               │
                                            reads
                                               ▼
...
Runs once on every Gemini CLI session start. Does three things:
1. Writes `"full"` to `~/.Gemini/.caveman-active` (creates it if missing)
2. Emits the caveman ruleset as hidden stdout — Gemini CLI injects SessionStart hook stdout as system context, invisible to the user
3. Checks `~/.Gemini/settings.json` for an existing statusline config; if missing, appends a nudge telling Gemini to offer setup on first interaction

Silent-fails on all filesystem errors — never blocks session start.

### `hooks/caveman-mode-tracker.js` — UserPromptSubmit hook

Reads JSON from stdin (Gemini CLI passes prompt data as JSON on this hook event). Checks if the user prompt starts with `/caveman`. If yes, writes the detected mode to the flag file:
- `/caveman` → `full`
- `/caveman lite` → `lite`
- `/caveman ultra` → `ultra`
...
Reads the flag file. Outputs a colored badge string for the Gemini CLI statusline:
- `full` or empty → `[CAVEMAN]` (orange)
- anything else → `[CAVEMAN:<MODE_UPPERCASED>]` (orange)

Configured in `~/.Gemini/settings.json` under `statusLine.command`.

### Hook installation

**Plugin install** — hooks are wired automatically by the plugin system.

**Standalone install** — `hooks/install.sh` (macOS/Linux) or `hooks/install.ps1` (Windows) copies the three hook files into `~/.Gemini/hooks/` and patches `~/.Gemini/settings.json` to register SessionStart and UserPromptSubmit hooks plus the statusline.
...
Skills are Markdown files with YAML frontmatter consumed by Gemini CLI's skill/plugin system and by `npx skills` for other agents.
...
`llm_run.py` calls `Gemini -p --system-prompt ...` per (prompt, arm), saves output to `evals/snapshots/results.json`. `measure.py` reads the snapshot offline with tiktoken (OpenAI BPE — approximates Gemini's tokenizer, ratios are meaningful, absolute numbers are approximate).
...
`benchmarks/` runs real prompts through the Gemini API (not Gemini CLI CLI) and records raw token counts. Results are committed as JSON in `benchmarks/results/`. The benchmark table in README is generated from these results — update it when regenerating.
