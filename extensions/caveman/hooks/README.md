...
- Writes `full` to `~/.Gemini/.caveman-active` (flag file)
- Emits caveman rules as hidden SessionStart context
- Detects missing statusline config and emits setup nudge (Gemini will offer to help)
...
- Reads `~/.Gemini/.caveman-active` and outputs a colored badge
...
**Plugin users:** If you do not already have a `statusLine` configured, Gemini will detect that on your first session after install and offer to set it up for you. Accept and you're done.

If you already have a custom statusline, caveman does not overwrite it and Gemini stays quiet. Add the badge snippet to your existing script instead.
...
**Manual setup:** If you need to configure it yourself, add one of these to `~/.Gemini/settings.json`:
...
Replace the path with the actual script location (e.g. `~/.Gemini/hooks/` for standalone installs, or the plugin install directory for plugin installs).

**Custom statusline:** If you already have a statusline script, add this snippet to it:

```bash
caveman_text=""
caveman_flag="$HOME/.Gemini/.caveman-active"
...
SessionStart stdout is injected as hidden system context — Gemini sees it, users don't. The statusline runs as a separate process. The flag file is the bridge.
...
Or manually:
1. Remove `~/.Gemini/hooks/caveman-activate.js`, `~/.Gemini/hooks/caveman-mode-tracker.js`, and the matching statusline script (`caveman-statusline.sh` on macOS/Linux or `caveman-statusline.ps1` on Windows)
2. Remove the SessionStart, UserPromptSubmit, and statusLine entries from `~/.Gemini/settings.json`
3. Delete `~/.Gemini/.caveman-active`
