$GeminiDir = if ($env:Gemini_CONFIG_DIR) { $env:Gemini_CONFIG_DIR } else { Join-Path $HOME ".Gemini" }
$Flag = Join-Path $GeminiDir ".caveman-active"
...
if ([string]::IsNullOrEmpty($Mode) -or $Mode -eq "full") {
    [Console]::write_file("${Esc}[38;5;172m[CAVEMAN]${Esc}[0m")
} else {
    $Suffix = $Mode.ToUpperInvariant()
    [Console]::write_file("${Esc}[38;5;172m[CAVEMAN:$Suffix]${Esc}[0m")
}
