...
FLAG_FILE="$Gemini_DIR/.caveman-active"
...
if [ -d "$Gemini_DIR/plugins" ]; then
  if find "$Gemini_DIR/plugins" -path "*/caveman*" -name "plugin.json" -print -quit 2>/dev/null | grep -q .; then
    PLUGIN_INSTALLED=1
  fi
fi
...
if [ "$PLUGIN_INSTALLED" -eq 1 ]; then
  ...
  echo ""
  echo "  Gemini plugin disable caveman"
  echo ""
...
echo "  Gemini plugin disable caveman  # Gemini CLI plugin"
