...
$GeminiDir = if ($env:Gemini_CONFIG_DIR) { $env:Gemini_CONFIG_DIR } else { Join-Path $env:USERPROFILE ".Gemini" }
$HooksDir = Join-Path $GeminiDir "hooks"
$Settings = Join-Path $GeminiDir "settings.json"
$FlagFile = Join-Path $GeminiDir ".caveman-active"
...
$PluginsDir = Join-Path $GeminiDir "plugins"
...
if ($PluginInstalled) {
    ...
    Write-Host ""
    Write-Host "  Gemini plugin disable caveman" -ForegroundColor Cyan
    Write-Host ""
...
Write-Host "  Gemini plugin disable caveman   # Gemini CLI plugin"
