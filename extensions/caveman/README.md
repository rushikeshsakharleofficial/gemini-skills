...
| Agent | Install |
|-------|---------|
| **Gemini CLI** | `Gemini plugin marketplace add JuliusBrussee/caveman && Gemini plugin install caveman@caveman` |
...
Auto-activation is built in for Gemini CLI, Gemini CLI, and the repo-local Codex setup below. `npx skills add` installs the skill for other agents, but does **not** install repo rule/instruction files, so Caveman does not auto-start there unless you add the always-on snippet below.

| Feature | Gemini CLI | Codex | Gemini CLI | Cursor | Windsurf | Cline | Copilot |
|---------|:-----------:|:-----:|:----------:|:------:|:--------:|:-----:|:-------:|
...
> [!NOTE]
> Auto-activation works differently per agent: Gemini CLI uses SessionStart hooks, this repo's Codex dogfood setup uses `.codex/hooks.json`, Gemini uses context files. Cursor/Windsurf/Cline/Copilot can be made always-on, but `npx skills add` installs only the skill, not the repo rule/instruction files.
>
...
The plugin install gives you skills + auto-loading hooks. If no custom `statusLine` is configured, Caveman nudges Gemini to offer badge setup on first session.

```bash
Gemini plugin marketplace add JuliusBrussee/caveman
Gemini plugin install caveman@caveman
```
...
- **Plugin install:** If you do not already have a custom `statusLine`, Gemini should offer to configure it on first session
...
`/caveman:compress <filepath>` — caveman make Gemini *speak* with fewer tokens. **Compress** make Gemini *read* fewer tokens.

Your `Gemini.md` loads on **every session start**. Caveman Compress rewrites memory files into caveman-speak so Gemini reads less — without you losing the human-readable original.

```
/caveman:compress Gemini.md
```
...
|------|----------:|----------:|------:|
| `Gemini-md-preferences.md` | 706 | 285 | **59.6%** |
| `project-notes.md` | 1145 | 535 | **53.3%** |
| `Gemini-md-project.md` | 1122 | 636 | **43.3%** |
...
Real token counts from the Gemini API ([reproduce it yourself](benchmarks/)):
...
The `evals/` directory has a three-arm eval harness that measures real token compression against a proper control — not just "verbose vs skill" but "terse vs skill". Because comparing caveman to verbose Gemini conflate the skill with generic terseness. That cheating. Caveman not cheat.

```bash
# Run the eval (needs Gemini CLI)
