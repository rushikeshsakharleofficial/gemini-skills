...
Requires:
  - `Gemini` CLI on PATH (Gemini CLI), authenticated

Run: uv run python evals/llm_run.py

Environment:
  CAVEMAN_EVAL_MODEL  optional --model flag value passed through to Gemini
"""
...
def run_Gemini(prompt: str, system: str | None = None) -> str:
    cmd = ["Gemini", "-p"]
    if system:
...
def Gemini_version() -> str:
    try:
        out = subprocess.run(
            ["Gemini", "--version"], capture_output=True, text=True, check=True
        )
        return out.stdout.strip()
    except Exception:
        return "unknown"
...
def main() -> None:
    ...
    snapshot["arms"]["__baseline__"] = [run_Gemini(p) for p in prompts]

    print("terse (control: terse instruction only, no skill)", flush=True)
    snapshot["arms"]["__terse__"] = [
        run_Gemini(p, system=TERSE_PREFIX) for p in prompts
    ]
    ...
    for skill in skills:
        ...
        snapshot["arms"][skill] = [run_Gemini(p, system=system) for p in prompts]
