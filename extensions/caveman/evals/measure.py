...
approximation of Gemini's BPE. The ratios are still meaningful for
comparing skills against each other, but the absolute numbers should be
read as "approximate output-length reduction", not "exact Gemini tokens".
...
def main() -> None:
    ...
    print(
        f"_Model: {meta.get('model', '?')} · CLI: {meta.get('Gemini_cli_version', '?')}_"
    )
    print(f"_Tokenizer: tiktoken o200k_base (approximation of Gemini's BPE)_")
    print(
